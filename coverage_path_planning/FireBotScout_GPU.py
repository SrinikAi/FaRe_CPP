...
class Exploration:
    ...
    def line_of_sight(self, grid_map, start, end, unoccupied_value):
        """Determine if there is a clear line of sight between two points in the grid."""
        x0, y0 = start
        x1, y1 = end
        dx = abs(x1 - x0)
        dy = -abs(y1 - y0)
    ...
    def increment_cells_within_circle_tf(self, grid_map, start_position, radius, unoccupied_value):
        ...
        # Initialize the increment mask with False values
        increment_mask = tf.zeros_like(grid_map_tensor, dtype=tf.bool)

        # Update the increment mask based on conditions
        increment_mask = tf.logical_and(mask_within_circle, tf.equal(grid_map_tensor, unoccupied_value))
